...
pom.xml		
----
<1> Dockerfile for AsciidoctorJ into WildFly 8.2
<2> Java Class which convert AsciiDoc file into HTML or PDf file
<3> Java Servlet which used the converter
<4> Sample AsciiDoc file
<5> Tests to check that AsciidoctorJ works
<6> MANIFEST file to depend on AsciidoctorJ WilFly AS module
<7> XML file to configure Arquillian tests
====
